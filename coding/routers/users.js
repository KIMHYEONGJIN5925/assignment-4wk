...
const postUsersSchema = Joi.object({     //스키마정의하기 , 유효성검사
...
router.post("/users", async (req, res) => { // 포스트메소드, 경로는 /users 
    try {
        const { nickname, password, confirmPassword } = await postUsersSchema.validateAsync(req.body); // Joi스키마 정의된 것으로 유효성검증
...
router.post("/auth", async (req, res) => { //로그인api 작성준비
    try {
        ...
        if(!user) { //유저가없다면
            ...
            errorMessage: '불친절하게 닉네님과 비밀번호 둘중 하나가 잘못되었습니다.', // 불친절하게 다알려주지 말자
            }); 
            return; // 에러났으면 리턴해야함
        }
